...
const EditExpense = () => {
  ...
  const normalizeMap = (mapLike: any): Record<string, string> => {
    if (!mapLike) return {};
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.success && response.data) {
          ...
          const allMembers = normalizeMembers(tripData, payload.membersWithBalances || []);
          setMembers(allMembers);
          
          // Find the expense to edit
          const expenses = payload.expenses || [];
          const expense = expenses.find((e: any) => (e._id || e.id) === expenseId);
          if (expense) {
            const paidByValue = getIdValue(expense.paidBy);
            const splitIds = (expense.splitBetween || []).map((m: any) => getIdValue(m));
            ...
            if (expense.splitType) {
              ...
              if (expense.splitType === "unequally" && expense.splitAmounts) {
                setCustomAmounts(normalizeMap(expense.splitAmounts));
              } else if (expense.splitType === "percentage" && expense.splitPercentages) {
                setPercentages(normalizeMap(expense.splitPercentages));
              } else if (expense.splitType === "shares" && expense.splitShares) {
                setShares(normalizeMap(expense.splitShares));
              } else {
  ...
  const handleUpdateExpense = async () => {
    ...
    try {
      ...
      const response = await backendCall(`/expenses/${expenseId}`, {
        method: "PUT",
