...
export default function DiagnosticPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      backendCall(`/trips/${id}`),
      backendCall(`/trips/${id}/settlements`)
    ]).then(([trip, settlements]) => {
      const payload = trip.data || {};
      const tripData = payload.trip || payload;
      const members = payload.membersWithBalances || tripData.members || [];

      setTripData({
        ...tripData,
        members,
        totalExpense: payload.totalTripExpense || 0,
      });
