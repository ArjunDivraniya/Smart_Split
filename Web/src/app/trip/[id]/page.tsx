...
const TripOverview = () => {
  ...
  const { call: backendCall } = useBackendAPI();
  ...
  const getMapValue = (mapLike: any, key: string) => {
    if (!mapLike || !key) return undefined;
    if (mapLike instanceof Map) return mapLike.get(key);
    if (typeof mapLike === "object" && key in mapLike) return mapLike[key];
    return undefined;
  };

  const computeShareAmount = (expense: any, memberId: string, splitIds: string[], totalAmount: number) => {
    const amountOverride = getMapValue(expense.splitAmounts, memberId);
    if (amountOverride !== undefined) return Number(amountOverride) || 0;

    const percent = getMapValue(expense.splitPercentages, memberId);
    if (percent !== undefined) return (totalAmount * Number(percent)) / 100;

    const shares = getMapValue(expense.splitShares, memberId);
    if (shares !== undefined) {
      const totalShares = splitIds.reduce((sum, id) => sum + (Number(getMapValue(expense.splitShares, id)) || 0), 0);
      return totalShares ? (totalAmount * Number(shares)) / totalShares : 0;
    }

    return splitIds.length ? totalAmount / splitIds.length : 0;
  };
  ...
  const performSearch = async (query: string) => {
    setIsSearching(true);
    try {
      const response = await backendCall(`/user/search?query=${encodeURIComponent(query)}`);
      const users = response.success ? response.data : (Array.isArray(response.data) ? response.data : []);
  ...
  const fetchTripDetails = async () => {
    ...
    try {
      const response = await backendCall(`/trips/${id}`);
      if (!response.success) throw new Error(response.message || "Failed to load trip");

      const payload = response.data || {};
      const tripData = payload.trip || payload;
      const expenses = payload.expenses || [];
      const totalTripExpense = payload.totalTripExpense;
      const membersWithBalances = payload.membersWithBalances || [];
      ...
      const creatorId = getIdValue(tripData.createdBy);
      const statusByUserId = new Map<string, string>();
      (tripData.members || []).forEach((m: any) => {
        const uid = getIdValue(m.userId);
        if (uid) statusByUserId.set(uid, m.status || "invited");
      });
      if (creatorId) statusByUserId.set(creatorId, "joined");

      const normalizedMembers = (membersWithBalances.length ? membersWithBalances : (tripData.members || []).map((m: any) => ({
        userId: getIdValue(m.userId),
        ...
        status: statusByUserId.get(m.userId) || "joined",
        balance: Number(m.balance || 0),
      }));

      const normalizedExpenses = expenses.map((expense: any) => {
        const amount = Number(expense.amount || 0);
        const payerId = getIdValue(expense.paidBy);
        const splitUsers = expense.splitBetween || [];
        const splitIds = splitUsers.map((u: any) => getIdValue(u)).filter(Boolean) as string[];
        const splitNames = splitUsers.map((u: any) => u?.name).filter(Boolean).join(", ");
        const breakdown = splitUsers.map((u: any) => {
          const uid = getIdValue(u) || "";
          return {
            id: uid,
            name: u?.name || "Unknown",
            amount: computeShareAmount(expense, uid, splitIds, amount),
          };
        });

        const yourShare = currentUserId && splitIds.includes(currentUserId)
          ? computeShareAmount(expense, currentUserId, splitIds, amount)
          : undefined;

        let statusLabel = "Not involved";
        let rightAmount: number | null = null;

        if (currentUserId) {
          if (payerId === currentUserId) {
            const diff = amount - (yourShare || 0);
            rightAmount = diff;
            statusLabel = diff >= 0 ? "You lent" : "You borrowed";
          } else if (splitIds.includes(currentUserId)) {
            rightAmount = yourShare || 0;
      ...
      const currentBalance = membersWithBalances.find((m: any) => m.userId === currentUserId)?.balance || 0;
      const fallbackTotalExpense = normalizedExpenses.reduce((sum: number, e: any) => sum + (Number(e.amount) || 0), 0);

      setTrip({
        id: tripData._id || tripData.id,
        name: tripData.name,
        location: tripData.destination || tripData.location,
        startDate: tripData.startDate,
        endDate: tripData.endDate,
        status: tripData.status,
        isCreator: creatorId && currentUserId ? creatorId === currentUserId : false,
        currentUserId: currentUserId,
        members: normalizedMembers,
        expenses: normalizedExpenses,
        totalExpense: Number(totalTripExpense || 0) || fallbackTotalExpense,
        yourBalance: Number(currentBalance || 0),
      });
    } catch (error) {
      console.error(error);
  ...
  const fetchSettlements = async () => {
    ...
    try {
      const response = await backendCall(`/trips/${id}/settlements`);
      if (!response.success) throw new Error(response.message || "Failed to load settlements");
  ...
  const handleEndTrip = async () => {
    setEnding(true);
    try {
      const response = await backendCall(`/trips/${id}/end`, { method: "POST" });
      if (response.success) {
        toast.success("Trip marked as completed");
        setTrip((prev: any) => ({ ...prev, status: "completed" })); 
      } else {
        toast.error(response.message || "Only admin can end trip");
      }
    } catch (e) {
  ...
  const handleAddMember = async () => {
    ...
    try {
      const response = await backendCall(`/trips/${id}/add-member`, {
        method: "POST",
  ...
  const handleDeleteExpense = async (expenseId: string) => {
    ...
    try {
      const response = await backendCall(`/expenses/${expenseId}`, { method: "DELETE" });
      if (response.success) {
