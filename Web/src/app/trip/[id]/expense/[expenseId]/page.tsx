...
const ExpenseDetail = () => {
  ...
  const normalizeMembers = (tripData: any, membersWithBalances: any[]) => {
    if (membersWithBalances && membersWithBalances.length) {
      return membersWithBalances.map((m: any) => ({
        id: m.userId,
        name: m.name,
    ...
    return (tripData?.members || []).map((m: any) => ({
      id: getIdValue(m.userId),
  ...
  useEffect(() => {
    ...
    const fetchExpenseDetail = async () => {
      try {
        const response = await backendCall(`/trips/${tripId}`);
        ...
        if (response.success && response.data) {
          ...
          setMembers(normalizeMembers(tripData, payload.membersWithBalances || []));
          const rawExpense = expenses.find((e: any) => (e._id || e.id) === expenseId);
          
          if (rawExpense) {
            const amount = Number(rawExpense.amount || 0);
            const payerId = getIdValue(rawExpense.paidBy);
            const splitUsers = rawExpense.splitBetween || [];
            ...
            const breakdown = splitUsers.map((u: any) => {
              ...
              return {
                id: uid,
                name: u?.name || "Unknown",
                amount: computeShareAmount(rawExpense, uid, splitIds, amount),
              };
            });

            const yourShare = currentUserId && splitIds.includes(currentUserId)
              ? computeShareAmount(rawExpense, currentUserId, splitIds, amount)
              : undefined;
            ...
            setExpense({
              id: rawExpense._id || rawExpense.id,
              title: rawExpense.title,
              amount: amount,
              category: rawExpense.category || "other",
              paidBy: rawExpense.paidBy?.name || "Unknown",
              paidById: payerId,
              date: rawExpense.date || rawExpense.createdAt,
              splitType: rawExpense.splitType || "equally",
              breakdown,
              statusLabel,
              yourShare,
            });
