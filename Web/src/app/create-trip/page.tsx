...
const CreateTrip = () => {
  ...
  const handleCreateTrip = async () => {
    ...
    try {
      const response = await backendCall("/trips/create", {
        method: "POST",
        body: JSON.stringify({
          name: tripName,
      ...
      // Get trip ID from response - could be tripId or data._id depending on backend response
      const tripId = (response.data as any)?._id || (response.data as any)?.tripId;
      if (!tripId) {
        throw new Error("Trip created, but no trip ID returned");
      }
      router.push(`/trip/${tripId}`); // Redirect to new trip
