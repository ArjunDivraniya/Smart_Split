...
const Dashboard = () => {
  ...
  // Redirect to login if not authenticated (after checking session loaded)
  useEffect(() => {
    if (status === "loading") {
  ...
  const fetchData = async () => {
    try {
      ...
      // Fetch User Trips from Express Backend
      const tripData = await backendCall("/trips/user");
      if (tripData.success) {
  ...
  const handleResponse = async (tripId: string, action: "accept" | "reject") => {
    try {
      const response = await backendCall(`/trips/${tripId}/respond`, {
        method: "POST",
